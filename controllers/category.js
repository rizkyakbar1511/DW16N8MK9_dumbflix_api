...
exports.readCategory = async (req,res) =>{
    try {
        const category = await Category.findAll({ attributes: { exclude: ["createdAt", "updatedAt"] } });
...
exports.editCategory = async (req,res) => {
    try {
        ...
        if(!categoryUpdated) return res.status(400).send({
            status : "Failed",
...
try {
    ...
    return res.status(200).send({status:"Success",message: `Category with id : ${id} deleted successfully`});
} catch (error) {
    console.log(error);
    return res.status(500).send({error: "Internal Server Error"});
}
